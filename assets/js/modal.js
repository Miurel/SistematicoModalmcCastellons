...
function ShowModalForEdit(sender){
	...
	//
	var ProfesionesString = table.rows[SelectedRowIndex].cells[4].innerHTML.toString();
	var ProfesionesArray = ProfesionesString.split(",");
	for(var x=0;x<ProfesionesArray.length;x++)
	{
        SelectCheckBoxByValue("Profesiones",ProfesionesArray[x].trim());
	}	
...
function ActualizarRegistro(){
    if (ValidarCedula(document.getElementById('IDTextBox').value)){
        ...
        table.rows[SelectedRowIndex].cells[4].innerHTML=profesiones;      
    }
    modal.style.display = "none";
}